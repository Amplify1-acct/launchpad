...
function buildStarterSite(bizType: string, industry: string, city: string, phone: string, ai: AIContent): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap" rel="stylesheet"/><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Inter',sans-serif;color:#111;background:#fff;}nav{padding:18px 48px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #f0f0f0;background:#fff;}.logo{font-size:18px;font-weight:900;}.cta-btn{background:#111;color:#fff;padding:10px 22px;border-radius:8px;font-size:13px;font-weight:700;}.hero{position:relative;height:88vh;overflow:hidden;background:#1a1a2e;}.hero-overlay{position:absolute;inset:0;background:linear-gradient(to right,rgba(0,0,0,0.80) 55%,rgba(0,0,0,0.1));}.hero-content{position:absolute;inset:0;display:flex;flex-direction:column;justify-content:center;padding:0 72px;}.tag{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:rgba(255,255,255,0.5);margin-bottom:16px;}h1{font-size:52px;font-weight:900;color:#fff;line-height:1.05;letter-spacing:-2px;max-width:560px;margin-bottom:18px;}.sub{font-size:16px;color:rgba(255,255,255,0.7);max-width:460px;line-height:1.75;margin-bottom:28px;}.btns{display:flex;gap:12px;}.btn-w{background:#fff;color:#111;padding:13px 26px;border-radius:8px;font-size:14px;font-weight:700;}.btn-g{border:2px solid rgba(255,255,255,0.3);color:#fff;padding:13px 22px;border-radius:8px;font-size:14px;font-weight:600;}.services{padding:64px 72px;background:#f9f9f9;}.sec-tag{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:#999;margin-bottom:8px;}.sec-h{font-size:28px;font-weight:800;letter-spacing:-0.5px;margin-bottom:32px;}.svc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;}.svc{background:#fff;border-radius:10px;padding:22px;border:1px solid #f0f0f0;}.svc-dot{width:8px;height:8px;background:#111;border-radius:50%;margin-bottom:12px;}.svc-n{font-size:14px;font-weight:700;}.stats{display:grid;grid-template-columns:repeat(4,1fr);background:#111;}.stat{padding:28px;text-align:center;border-right:1px solid rgba(255,255,255,0.08);}.stat-n{font-size:26px;font-weight:900;color:#fff;}.stat-l{font-size:10px;color:rgba(255,255,255,0.4);margin-top:4px;text-transform:uppercase;letter-spacing:1px;}.cta-sec{padding:52px 72px;display:flex;justify-content:space-between;align-items:center;}.cta-sec h2{font-size:24px;font-weight:800;}.cta-ph{background:#111;color:#fff;padding:13px 26px;border-radius:8px;font-size:15px;font-weight:800;}footer{padding:20px 72px;background:#f5f5f5;display:flex;justify-content:space-between;font-size:11px;color:#999;}</style></head><body><nav><div class="logo">${bizType}</div><div class="cta-btn">${phone||"Call Us"}</div></nav><div class="hero"><div style="position:absolute;inset:0;">${heroEl}</div><div class="hero-overlay"></div><div class="hero-content"><div class="tag">${city} · ${bizType}</div><h1>${ai.headline}</h1><p class="sub">${ai.subtext}</p><div class="btns"><div class="btn-w">Get Free Estimate →</div><div class="btn-g">Our Services</div></div></div></div><section class="services"><div class="sec-tag">What We Offer</div><h2 class="sec-h">Our Services</h2><div class="svc-grid">${svcs.map(s=>`<div class="svc"><div class="svc-dot"></div><div class="svc-n">${s}</div></div>`).join("")}</div></section><div class="stats"><div class="stat"><div class="stat-n">${ai.stat1}</div><div class="stat-l">${ai.stat1Label}</div></div><div class="stat"><div class="stat-n">${ai.stat2}</div><div class="stat-l">${ai.stat2Label}</div></div><div class="stat"><div class="stat-n">4.9★</div><div class="stat-l">Rating</div></div><div class="stat"><div class="stat-n">Free</div><div class="stat-l">Estimates</div></div></div><section class="cta-sec"><div><h2>Ready to get started?</h2></div><div class="cta-ph">${phone||"Contact Us"}</div></section><footer><span>${bizType} · ${city}</span><span>Exsisto Starter · $99/mo</span></footer></body></html>`;
}
...
function buildProSite(bizType: string, industry: string, city: string, phone: string, ai: AIContent): string {
  ...
  const heroEl = hasImgs ? `<img src="${hero}" style="width:100%;height:100%;object-fit:cover;min-height:500px;"/>` : PLACEHOLDER_IMG_HTML;
  const svcs = ai.services.slice(0, 6);
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700;800;900&display=swap" rel="stylesheet"/><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Inter',sans-serif;color:#111;background:#fff;}nav{padding:18px 56px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #f0f0f0;background:#fff;}.logo{font-size:18px;font-weight:900;}.nav-links{display:flex;gap:24px;}.nav-links a{font-size:13px;font-weight:500;color:#666;text-decoration:none;}.cta-btn{background:#111;color:#fff;padding:10px 22px;border-radius:8px;font-size:13px;font-weight:700;}.hero{display:grid;grid-template-columns:45% 55%;min-height:88vh;}.hero-left{padding:80px 56px;display:flex;flex-direction:column;justify-content:center;}.tag{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#999;margin-bottom:18px;}h1{font-size:46px;font-weight:900;line-height:1.05;letter-spacing:-2px;margin-bottom:16px;}.sub{font-size:15px;color:#555;line-height:1.8;margin-bottom:24px;max-width:400px;}.stats-row{display:flex;gap:20px;padding:16px 20px;background:#f9f9f9;border-radius:10px;margin-bottom:28px;}.stat-n{font-size:20px;font-weight:900;}.stat-l{font-size:10px;color:#999;margin-top:2px;text-transform:uppercase;letter-spacing:1px;}.btns{display:flex;gap:10px;}.btn-d{background:#111;color:#fff;padding:13px 26px;border-radius:8px;font-size:14px;font-weight:700;}.btn-o{border:2px solid #e5e5e5;color:#333;padding:13px 20px;border-radius:8px;font-size:14px;font-weight:600;}.hero-right{position:relative;overflow:hidden;}.badge{position:absolute;bottom:24px;left:24px;background:rgba(255,255,255,0.95);padding:14px 18px;border-radius:10px;}.badge-n{font-size:20px;font-weight:900;}.badge-l{font-size:10px;color:#999;margin-top:2px;}.services{padding:64px 56px;background:#f9f9f9;}.sec-tag{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:#999;margin-bottom:8px;}.sec-h{font-size:28px;font-weight:800;letter-spacing:-0.5px;margin-bottom:32px;}.svc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;}.svc{background:#fff;border-radius:10px;padding:20px;}.svc-dot{width:8px;height:8px;background:#111;border-radius:50%;margin-bottom:10px;}.svc-n{font-size:13px;font-weight:700;}.gallery{padding:64px 56px;}.gal-grid{display:grid;grid-template-columns:2fr 1fr 1fr;gap:12px;margin-top:24px;}.gal-grid img{width:100%;height:240px;object-fit:cover;border-radius:10px;background:#eee;display:block;}.cta-sec{background:#111;padding:52px 56px;display:flex;justify-content:space-between;align-items:center;}.cta-sec h2{font-size:24px;font-weight:800;color:#fff;}.cta-ph{background:#fff;color:#111;padding:13px 26px;border-radius:8px;font-size:15px;font-weight:800;}footer{padding:20px 56px;background:#f0f0f0;display:flex;justify-content:space-between;font-size:11px;color:#999;}</style></head><body><nav><div class="logo">${bizType}</div><div class="nav-links"><a href="#">Services</a><a href="#">Gallery</a><a href="#">About</a></div><div class="cta-btn">${phone||"Call Now"}</div></nav><div class="hero"><div class="hero-left"><div class="tag">${city}</div><h1>${ai.headline}</h1><p class="sub">${ai.subtext}</p><div class="stats-row"><div><div class="stat-n">${ai.stat1}</div><div class="stat-l">${ai.stat1Label}</div></div><div><div class="stat-n">${ai.stat2}</div><div class="stat-l">${ai.stat2Label}</div></div><div><div class="stat-n">4.9★</div><div class="stat-l">Rating</div></div></div><div class="btns"><div class="btn-d">Get Free Estimate →</div><div class="btn-o">View Our Work</div></div></div><div class="hero-right">${heroEl}<div class="badge"><div class="badge-n">★ 4.9</div><div class="badge-l">200+ Reviews</div></div></div></div><section class="services"><div class="sec-tag">What We Offer</div><h2 class="sec-h">Our Services</h2><div class="svc-grid">${svcs.map(s=>`<div class="svc"><div class="svc-dot"></div><div class="svc-n">${s}</div></div>`).join("")}</div></section><section class="gallery"><div class="sec-tag">Our Work</div><h2 class="sec-h">See the Results</h2><div class="gal-grid">${hasImgs?`<img src="${hero}"/><img src="${card1}"/><img src="${card2}"/>`:`${PLACEHOLDER_CARD(1)}${PLACEHOLDER_CARD(2)}${PLACEHOLDER_CARD(3)}`}</div></section><section class="cta-sec"><div><h2>Ready to get started?</h2></div><div class="cta-ph">${phone||"Contact Us"}</div></section><footer><span>${bizType} · ${city}</span><span>Exsisto Pro · $299/mo</span></footer></body></html>`;
}
...
function buildPremiumSite(bizType: string, industry: string, city: string, phone: string, ai: AIContent): string {
  ...
  const svcs = ai.services.slice(0, 6);
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700;800;900&display=swap" rel="stylesheet"/><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Inter',sans-serif;background:#0a0a14;color:#fff;}nav{padding:20px 56px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.08);background:rgba(10,10,20,0.96);}.logo{font-size:18px;font-weight:900;background:linear-gradient(135deg,#a78bfa,#818cf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}.nav-links{display:flex;gap:28px;}.nav-links a{font-size:13px;color:rgba(255,255,255,0.5);text-decoration:none;}.nav-cta{background:linear-gradient(135deg,#6366f1,#818cf8);color:#fff;padding:10px 22px;border-radius:8px;font-size:13px;font-weight:700;}.hero{display:grid;grid-template-columns:1fr 1fr;min-height:88vh;}.hero-left{padding:80px 56px;display:flex;flex-direction:column;justify-content:center;}.tag{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:#6366f1;margin-bottom:20px;display:flex;align-items:center;gap:8px;}.tag::before{content:'';width:20px;height:1px;background:#6366f1;}h1{font-size:48px;font-weight:900;line-height:1.05;letter-spacing:-2px;margin-bottom:18px;background:linear-gradient(135deg,#fff 60%,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}.sub{font-size:16px;color:rgba(255,255,255,0.6);line-height:1.8;margin-bottom:28px;max-width:420px;}.stats{display:flex;gap:20px;padding:18px 22px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;margin-bottom:28px;}.stat-n{font-size:20px;font-weight:900;color:#a78bfa;}.stat-l{font-size:10px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;margin-top:2px;}.btns{display:flex;gap:12px;}.btn-p{background:linear-gradient(135deg,#6366f1,#818cf8);color:#fff;padding:14px 28px;border-radius:8px;font-size:14px;font-weight:700;}.btn-s{border:1.5px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.8);padding:14px 22px;border-radius:8px;font-size:14px;}.hero-right{position:relative;overflow:hidden;}.hero-right img{width:100%;height:100%;object-fit:cover;min-height:500px;}.services{padding:64px 56px;background:#0d0d1a;}.sec-eyebrow{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:#6366f1;margin-bottom:8px;}.sec-h{font-size:28px;font-weight:800;letter-spacing:-0.5px;margin-bottom:28px;background:linear-gradient(135deg,#fff,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}.svc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;}.svc{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:22px;}.svc-dot{width:30px;height:30px;background:linear-gradient(135deg,#6366f1,#818cf8);border-radius:8px;margin-bottom:12px;}.svc-n{font-size:14px;font-weight:700;color:#fff;}.gallery{padding:64px 56px;background:#0a0a14;}.gal-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-top:24px;}.gal-grid img{width:100%;height:220px;object-fit:cover;border-radius:10px;}.cta{padding:56px;background:linear-gradient(135deg,#1e1b4b,#312e81);text-align:center;}.cta h2{font-size:32px;font-weight:900;letter-spacing:-1px;margin-bottom:10px;}.cta p{font-size:15px;color:rgba(255,255,255,0.65);margin-bottom:24px;}.cta-btn{background:#fff;color:#312e81;padding:14px 32px;border-radius:8px;font-size:15px;font-weight:800;display:inline-block;}footer{padding:22px 56px;background:#050509;display:flex;justify-content:space-between;font-size:11px;color:rgba(255,255,255,0.25);}</style></head><body><nav><div class="logo">${bizType}</div><div class="nav-links"><a href="#">Services</a><a href="#">Work</a><a href="#">About</a></div><div class="nav-cta">${phone||"Contact Us"}</div></nav><div class="hero"><div class="hero-left"><div class="tag">${city||"Your City"}</div><h1>${ai.headline}</h1><p class="sub">${ai.subtext}</p><div class="stats"><div><div class="stat-n">${ai.stat1}</div><div class="stat-l">${ai.stat1Label}</div></div><div><div class="stat-n">${ai.stat2}</div><div class="stat-l">${ai.stat2Label}</div></div><div><div class="stat-n">4.9★</div><div class="stat-l">Rating</div></div></div><div class="btns"><div class="btn-p">Get Started →</div><div class="btn-s">View Our Work</div></div></div><div class="hero-right"><img src="${img1}" onerror="this.style.background='#1a1a2e';this.removeAttribute('src')"/></div></div><section class="services"><div class="sec-eyebrow">What We Offer</div><h2 class="sec-h">Our Services</h2><div class="svc-grid">${svcs.map((s:string)=>`<div class="svc"><div class="svc-dot"></div><div class="svc-n">${s}</div></div>`).join("")}</div></section><section class="gallery"><div class="sec-eyebrow">Our Work</div><h2 class="sec-h">See the Results</h2><div class="gal-grid"><img src="${img1}"/><img src="${img2}"/><img src="${img3}"/></div></section><section class="cta"><h2>Ready to work together?</h2><p>${bizType} · ${city||"Your City"}</p><div class="cta-btn">${phone||"Contact Us Today"}</div></section><footer><span>${bizType} · ${city||"Your City"}</span><span>Exsisto Premium · $599/mo</span></footer></body></html>`;
}
